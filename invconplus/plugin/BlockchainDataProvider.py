...
from invconplus.const import CACHE_DIR 
...
class BlockchainDataProvider(Provider):
    ...
    def _read(self, export_file):
        ...
        if os.path.exists(export_file) and ENABLE_CACHE:
            logging.debug(f"cached file exists {export_file}")
            ...
            logging.debug(f"cached file not exists {export_file}")
            scProvider =  SourcecodeProvider(self.params)
    ...
    def read(self, export_dir = f"{CACHE_DIR}/"):
